...
Assert-Condition ($targetedExpression -eq "`$_Order.exists and (`$_Order.id == 'MSX4_lib_IdleReturnHome' or @`$_Order.`$MSX4_IDLE_RETURN_HOME)") 'Timeout cleanup must select only the idle parent or explicitly tagged idle children.'
...
Assert-Condition ((Get-DocumentOrder $targetedLoop) -lt (Get-DocumentOrder $managerShipLoop.SelectSingleNode("./remove_value[@name='global.`$MSX4_SL_IdleShips.{`$_Ship}']"))) 'The idle stack must be cancelled before its timer entry is removed.'
...
function Assert-TseCycleContract {
    ...
    $escapeCall = $Document.SelectSingleNode("//attention[@min='unknown']/actions/run_script[@name=`"'msx4.lib.EscapeTravelBlacklist'`"]")
    $candidateCall = $Document.SelectSingleNode("//attention[@min='unknown']/actions/run_script[@name=`"'msx4.tde.GetTradeDataToUpdate'`"]")
    $idleLabel = $Document.SelectSingleNode("//attention[@min='unknown']/actions/label[@name='IDLE_LBL']")
    $idleCreate = $Document.SelectSingleNode("//attention[@min='unknown']/actions/create_order[@id=`"'MSX4_lib_IdleReturnHome'`" and @immediate='true']")
    ...
    Assert-Condition ($Document.SelectNodes("//create_order[@id=`"'MSX4_lib_IdleReturnHome'`"]").Count -eq 1) "$Mode must retain exactly one idle fallback creation site."
...
Assert-TseCycleContract -Document $sector -Mode 'TDE Sector' -TraceSource 'tde_s'
Assert-TseCycleContract -Document $galaxy -Mode 'TDE Galaxy' -TraceSource 'tde_g'
Write-Output '4-5/24 cancelling the immediate idle stack exposes TDE-S/G, whose restart begins at a full observable main cycle: OK'
...
foreach ($document in @($sector, $galaxy)) {
    ...
    Assert-Condition ($null -ne $escapeResultGuard) 'TDE must accept not_required without entering escape backoff.'
}
...
$sectorCandidateCall = $sector.SelectSingleNode("//run_script[@name=`"'msx4.tde.GetTradeDataToUpdate'`"]")
$galaxyCandidateCall = $galaxy.SelectSingleNode("//run_script[@name=`"'msx4.tde.GetTradeDataToUpdate'`"]")
Assert-Condition ($null -ne $sectorCandidateCall.SelectSingleNode("./param[@name='SECTOR' and @value='`$SECTOR']")) 'TDE-S candidate search must remain limited to its configured sector.'
Assert-Condition ($null -eq $galaxyCandidateCall.SelectSingleNode("./param[@name='SECTOR']")) 'TDE-G candidate search must not be limited to the current or idle-dock sector.'
...
Assert-Condition ($null -ne $galaxySectorSearch -and $null -ne $cachedSectorSelection) 'TDE-G helper must retain cached galaxy-wide discovery.'
Assert-Condition ($null -ne $targets.SelectSingleNode("//run_script[@name=`"'msx4.lib.SortByEstimatedTravelTime'`" and param[@name='LIST' and @value='`$_ReachableSectors']]")) 'TDE-G must retain travel-time ranking of reachable sectors.'
Assert-Condition ($null -ne $targets.SelectSingleNode("//do_if[@value='`$_FoundStations.count']/set_value[@name='`$SECTOR' and @exact='`$_Sector']")) 'TDE-G must continue deriving the selected sector from actual station candidates.'
Write-Output '9-11/24 TDE-S remains sector-bound; TDE-G remains galaxy-wide and may select another candidate sector: OK'

foreach ($document in @($sector, $galaxy)) {
    $candidateCall = $document.SelectSingleNode("//run_script[@name=`"'msx4.tde.GetTradeDataToUpdate'`"]")
    $idleCreate = $document.SelectSingleNode("//create_order[@id=`"'MSX4_lib_IdleReturnHome'`"]")
...
Write-Output 'TDE idle timeout return validation: PASS'
