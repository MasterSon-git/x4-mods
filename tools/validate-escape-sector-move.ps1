...
Assert-Param -Call $escapeMove -Name 'useknownpath' -Value '$USE_KNOWN_PATH' -Context 'Escape move'
Assert-Param -Call $escapeMove -Name 'recallsubordinates' -Value 'false' -Context 'Escape move'
Assert-Condition ($null -eq $escapeMove.SelectSingleNode("./param[@name='endintargetsector']")) 'Escape direct-Sector move must not set endintargetsector.'
...
Assert-Condition ($null -eq $updateMove.SelectSingleNode("./param[@name='endintargetsector']")) 'The normal TSE direct-Sector move must not set endintargetsector.'
Assert-Param -Call $updateMove -Name 'strictblacklist' -Value 'true' -Context 'Normal TSE move'
Assert-Param -Call $updateMove -Name 'useknownpath' -Value 'true' -Context 'Normal TSE move'
Assert-Param -Call $updateMove -Name 'recallsubordinates' -Value 'false' -Context 'Normal TSE move'
Assert-Condition ($null -eq $updateMove.SelectSingleNode("./param[@name='position']")) 'The normal TSE direct-Sector move must remain positionless.'
...
Assert-Param -Call $idleMove -Name 'useknownpath' -Value '$USE_KNOWN_PATH' -Context 'Idle move'
Assert-Param -Call $idleMove -Name 'recallsubordinates' -Value 'false' -Context 'Idle move'
Assert-Condition ($null -eq $idleMove.SelectSingleNode("./param[@name='endintargetsector']")) 'Idle Sector-plus-position move must remain free of endintargetsector.'
...
foreach ($pathCheck in $escapeRouteChecks) {
    ...
    Assert-Condition ($pathCheck.GetAttribute('useknownpath') -eq '$USE_KNOWN_PATH') 'Escape route checks must retain useknownpath.'
...
foreach ($normalDocument in @($getTargets, $update, $idle)) {
    foreach ($pathCheck in @($normalDocument.SelectNodes('//get_global_path'))) {
        Assert-Condition ($pathCheck.GetAttribute('useblacklist') -eq '$blacklistgroup') 'Every reviewed normal TSE/Idle global path must retain the effective blacklist.'
    }
}
...
Write-Output 'Normal TSE direct-Sector move contract: OK'
