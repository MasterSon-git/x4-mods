...
$tdeRoot = Join-Path $modsRoot 'MSX4_TradeDataExplorer'
...
$assistDiff = Read-XmlDocument (Join-Path $tdeRoot 'aiscripts/order.assist.xml')
...
$experienceFiles = @(Get-ChildItem -LiteralPath $tdeRoot -Recurse -File -Filter 'experiences.xml')
$experienceActions = @(
...
$customExperienceRefs = @(
    Get-ChildItem -LiteralPath $modsRoot -Recurse -File -Filter '*.xml' |
        Select-String -Pattern 'JP_SHIP_TRADESUBSCRIPTION_UPDATED|MSX4_SHIP_TRADE_DATA_UPDATED'
)
Assert-Condition ($experienceFiles.Count -eq 0) 'The unused TDE custom experiences library must be removed.'
Assert-Condition ($experienceActions.Count -eq 0) 'TDE and ScriptLibrary must not award experience for movement, idle, failure, or trade-information refresh.'
Assert-Condition ($customExperienceRefs.Count -eq 0) 'The retired custom experience ID is still referenced.'
Write-Output '1/5 custom TDE experience definition and all award actions are absent: OK'
...
Assert-Condition ($sectorSkill -eq '20') 'TDE-S required skill must remain 20.'
Assert-Condition ($galaxySkill -eq '40') 'TDE-G required skill must remain the visible two-star value 40.'
...
Write-Output '3/5 TDE-S/TDE-G use visible one-/two-star thresholds 20/40, matching Vanilla Update/Recon thresholds: OK'
...
$tdeInsertion = if ($tdeAdd.Count -eq 1) {
    $tdeAdd[0].SelectSingleNode("do_if[@value=`"`$orderdef.`$id == 'MSX4_TradeDataExplorerG'`"]")
}
Assert-Condition ($null -ne $skillGuard) 'Vanilla Assist required-skill guard is missing around the extension anchor.'
Assert-Condition ($tdeAdd.Count -eq 1 -and $null -ne $tdeInsertion) 'The TDE-G Mimic branch must remain attached inside the Vanilla compatibility chain.'
Write-Output '4/5 Mimic executes the TDE-G branch only behind Vanilla required-skill validation: OK'
...
Write-Output 'TDE experience and skill contract validation: PASS'
