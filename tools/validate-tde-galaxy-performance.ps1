...
$helperPath = Join-Path $repo 'mods\MSX4_TradeDataExplorer\aiscripts\msx4.tde.GetTradeDataToUpdate.xml'
...
Assert-True ($helperText.Contains('global.$MSX4_TDE_GalaxyCandidateStations') -and
...
Assert-True ($mdText.Contains('global.$MSX4_TDE_GalaxyCandidateEpoch + 1') -and
    $mdText.Contains('global.$MSX4_TDE_GalaxyCandidateState') -and
...
Assert-True ($helperText.Contains('global.$MSX4_TDE_SectorsBlacklistGroup') -and
    $helperText.Contains('global.$MSX4_TDE_ShipsCurrentSectorTable')) 'sector reservations still prevent duplicate work'
...
Assert-True ($galaxyText.Contains("`$_UpdateResult == 'success' or `$_UpdateResult == 'already_current'") -and
    $galaxyText.Contains('remove_from_list name="global.$MSX4_TDE_GalaxyCandidateStations"')) 'success removes the current target from the snapshot'
...
Assert-True ($sectorCacheNodes.Count -eq 0 -and $helperText.Contains('<do_if value="$SECTOR == null">')) 'TDE-S remains outside the galaxy cache path'
Assert-True ($helperText.Contains("defaultorder.id == 'Assist'") -and
    $galaxyText.Contains('global.$MSX4_TDE_GalaxyCandidateStations')) 'Mimic workers use the same coordinator contract'
foreach ($script in @($galaxy, $sector)) {
    Assert-True ($script.SelectNodes("//do_if[@value='not @`$_Ship.sector.exists']/wait[@exact='1s']").Count -eq 2) 'TDE waits through transient no-sector transitions before property access'
}
$idleManagerPath = Join-Path $repo 'mods\MSX4_ScriptLibrary\md\msx4.ScriptLibrary.md.xml'
...
Assert-True ($idleManagerText.Contains('$_Ship.orders.clone') -and
    ...
    -not ($idleManagerText -match 'MSX4_SL_ManageIdleReturnHome_MD[\s\S]*?<cancel_all_orders')) 'Bugfix 008 selective idle cleanup remains intact'
...
$idlePerformanceNodes = @($idle.SelectNodes("//debug_to_file[contains(@text, '[MSX4-SLIB-PERF]')]"))
...
if (-not $SkipExistingRegressions) {
    $existing = Get-ChildItem -LiteralPath $PSScriptRoot -Filter 'validate-*.ps1' |
        Where-Object { $_.Name -ne 'validate-tde-galaxy-performance.ps1' } |
        Sort-Object Name
...
Write-Host 'TDE Galaxy performance regression: PASS'
